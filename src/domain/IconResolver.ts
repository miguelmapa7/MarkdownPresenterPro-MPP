...
/** Modo de asignación de iconos */
export type IconMode = "content" | "varied";

export interface IIconResolver {
  resolve(metadata: SlideMetadata, index?: number): IconResult;
...
/** Iconos variados para el modo "varied" — cada diapositiva recibe uno diferente */
const VARIED_ICONS: string[] = [
  // Estrella
  ...
  // Bombilla
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="9" y1="18" x2="15" y2="18"/><line x1="10" y1="22" x2="14" y2="22"/><path d="M15.09 14c.18-.98.65-1.74 1.41-2.5A4.65 4.65 0 0 0 18 8 6 6 0 0 0 6 8c0 1 .23 2.23 1.5 3.5A4.61 4.61 0 0 1 8.91 14"/></svg>`,
  // Cohete
  ...
  // Bandera
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/></svg>`,
];
 ...
 *
 * Dos modos:
 * - "content": Icono basado en el tipo de contenido del archivo (code, image, table, text)
 * - "varied": Iconos diferentes cíclicamente para cada diapositiva
 */
export class IconResolver implements IIconResolver {
  private mode: IconMode = "varied";
  ...
  resolve(metadata: SlideMetadata, index: number = 0): IconResult {
    if (metadata.icon) {
      return this.loadCustomSvg(metadata.icon);
    }

    if (this.mode === "varied") {
      const icon = VARIED_ICONS[index % VARIED_ICONS.length]!;
      return { type: "default", content: icon, fallback: false };
    }
  ...
  loadCustomSvg(svgContent: string): IconResult {
    ...
    if (!trimmed.startsWith("<svg") || !trimmed.includes("</svg>")) {
      console.warn("[IconResolver] SVG personalizado inválido");
      return {
        type: "custom-svg",
        content: FALLBACK_ICON,
        fallback: true,
        warning: "SVG personalizado inválido",
      };
    }
    return { type: "custom-svg", content: trimmed, fallback: false };
  }
}
