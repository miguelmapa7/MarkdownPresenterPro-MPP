import React from "react";
import { useTheme } from "./ThemeProvider";
import { useTranslation } from "./I18nProvider";

interface ThemeToggleProps {
  className?: string;
}

/**
 * ThemeToggle — Botón de alternancia oscuro/claro.
 *
 * Muestra icono de sol (tema claro activo) o luna (tema oscuro activo).
 * Consume useTheme() para leer y cambiar el tema.
 */
export const ThemeToggle: React.FC<ThemeToggleProps> = ({ className = "" }) => {
  const { theme, toggleTheme } = useTheme();
  const { t } = useTranslation();

  return (
    <button
      onClick={toggleTheme}
      aria-label={theme === "light" ? t("theme.switchToDark") : t("theme.switchToLight")}
      title={theme === "light" ? t("theme.dark") : t("theme.light")}
      className={`p-2 rounded-lg transition-colors
        bg-gray-200/60 hover:bg-gray-300/80 text-gray-700
        dark:bg-gray-700/60 dark:hover:bg-gray-600/80 dark:text-gray-200
        ${className}`}
    >
      {theme === "light" ? (
        /* Sol — tema claro activo */
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="20"
          height="20"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        >
          <circle cx="12" cy="12" r="5" />
          <line x1="12" y1="1" x2="12" y2="3" />
          <line x1="12" y1="21" x2="12" y2="23" />
          <line x1="4.22" y1="4.22" x2="5.64" y2="5.64" />
          <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" />
          <line x1="1" y1="12" x2="3" y2="12" />
          <line x1="21" y1="12" x2="23" y2="12" />
          <line x1="4.22" y1="19.78" x2="5.64" y2="18.36" />
          <line x1="18.36" y1="5.64" x2="19.78" y2="4.22" />
        </svg>
      ) : (
        /* Luna — tema oscuro activo */
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="20"
          height="20"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        >
          <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" />
        </svg>
      )}
    </button>
  );
};
